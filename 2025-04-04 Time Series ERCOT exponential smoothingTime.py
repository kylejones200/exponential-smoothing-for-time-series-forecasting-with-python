...
def main():
    ...
    plt.plot(
        hold_out["date"], hold_out["values"], label="Hold-Out Values", color="Green"
    )
    ...
    model = ExponentialSmoothing(
        train["values"], trend="additive", seasonal="additive", seasonal_periods=24
    )
    ...
    mape_value = mean_absolute_percentage_error(
        hold_out["values"], hold_out["forecast"]
    )
